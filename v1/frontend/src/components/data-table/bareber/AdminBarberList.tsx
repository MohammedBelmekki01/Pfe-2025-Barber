...
interface AdminBarberListProps {
  reload?: number;
}

function AdminBarberList({ reload }: AdminBarberListProps) {
  ...
  const columns: ColumnDef<Barber>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        // For reload
        const [updateReload, setUpdateReload] = useState(0);

        // Handler to reload and close sheet
        const handleBarberUpdated = () => {
          setUpdateReload((r) => r + 1);
          setOpen(false);
          // If parent reload exists, call it by updating reload prop
          if (
            typeof window !== "undefined" &&
            typeof window.dispatchEvent === "function"
          ) {
            window.dispatchEvent(new CustomEvent("barber-updated"));
          }
        };
        ...
        return (
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent className="overflow-y-auto max-w-2xl w-full">
                <SheetHeader>
            ...
            <Button variant="secondary" className="w-full sm:w-auto">
              <Link
                to={`/admin/barber-details?id=${barber.id}`}
  ...
  useEffect(() => {
    ...
    // Listen for global reload event (for update)
    const reloadListener = () => {
      BarberApiii.allBarbers().then(({ data }) => {
        setData(data.data);
      });
    };
    window.addEventListener("barber-updated", reloadListener);
    return () => window.removeEventListener("barber-updated", reloadListener);
  }, [reload]);
